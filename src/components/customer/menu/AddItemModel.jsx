        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40 backdrop-blur-sm">
            <div className="bg-pastelred-300 rounded-xl p-6 w-[90%] max-w-md shadow-lg relative">
                ...
                <div className="flex justify-center items-center gap-4 mb-6">
                    <button onClick={decrease} className="w-10 h-10 rounded-full bg-white shadow text-xl">−</button>
                    <span className="text-xl">{quantity}</span>
                    <button onClick={increase} className="w-10 h-10 rounded-full bg-white shadow text-xl">+</button>
                </div>
                <button onClick={handleAdd} className="bg-white text-black font-semibold py-2 w-full rounded-full shadow">
                    ADD
                </button>
            </div>
        </div>
